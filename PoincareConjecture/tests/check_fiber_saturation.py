#!/usr/bin/env python3
"""Validate this contribution using the unchanged upstream dependency pins."""
...
repo = package.parent
out = (args.output or package/'.lake/fiber-saturation-audit').resolve()
...
text = run('axioms', ['lake', 'env', 'lean', 'tests/FiberSaturationAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 16, (len(closures), text)
...
report = dict(status='PASS', project_build_present_at_start=project_build_present_at_start, utc=datetime.now(timezone.utc).isoformat(),
              ...
              source_sha256={str(p.relative_to(package)):hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in sources},
              scope='Primary PoincareConjecture package; one blueprint node plus two topological interfaces; no full Poincare proof')
