#!/usr/bin/env python3
"""Validate the preceding proofs and the actual quotient-region classification."""
...
repo = package.parent
out = (args.output or package/'.lake/torus-region-audit').resolve()
...
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullTorusRegionAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 87, (len(closures), text)
...
report = dict(status='PASS', project_build_present_at_start=project_build_present_at_start, utc=datetime.now(timezone.utc).isoformat(),
              ...
              source_sha256={str(p.relative_to(package)):hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in sources},
              contribution_parent='9af0300d063e59a33d919818f7fd7eea79958bbc',
              scope='Actual quotient region: finite lifted strip, component image equals region, actual closure homeomorphism; regular-open bridge explicit; no arbitrary smooth sphere-bundle identification or complete Poincare theorem')
