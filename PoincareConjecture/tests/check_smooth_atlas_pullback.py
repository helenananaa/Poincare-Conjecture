#!/usr/bin/env python3
"""Validate smooth pullback charts, bundle-derived smooth presentation and boundary pairing."""
import argparse
import fcntl
import hashlib
import json
...
out = (a.output or package / '.lake/smooth-atlas-pullback-audit').resolve()
...
expected_parent = '5a85ceaf925e512a7f68767daed978283afaf6b2'
...
suites = [
    ...
    'AbstractBundleCutSanity', 'ClosedRealBridgeSanity', 'AbstractBoundaryPairingSanity', 'SmoothSpliceSanity', 'PeriodicEndMatchingSanity', 'IntegerExtensionSanity', 'CoverPullbackSanity', 'SmoothAtlasPullbackSanity',
]
for suite in suites:
    run(suite, ['lake', 'env', 'lean', 'tests/'+suite+'.lean'])
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullSmoothAtlasPullbackAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 348, (len(closures), text)
...
report = dict(
    ...
    scope='Standard smooth additive-circle atlas, actual pulled-back smooth bundle charts, smooth periodic presentation derived from original local charts, and smooth closed-cylinder plus boundary-component classification. Original total-space/fiber/closure structures and intrinsic boundary premises are retained. No complete Poincare proof.',
)
