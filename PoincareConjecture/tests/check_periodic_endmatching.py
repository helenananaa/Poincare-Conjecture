...
out = (a.output or package / '.lake/periodic-end-audit').resolve()
...
expected_parent = 'c92c159e8aa790d53ae64c8dfd561f7958a9f1b7'
...
suites = [
    ...
    'AbstractBundleCutSanity', 'ClosedRealBridgeSanity', 'AbstractBoundaryPairingSanity', 'SmoothSpliceSanity', 'PeriodicEndMatchingSanity',
]
for suite in suites:
    run(suite, ['lake', 'env', 'lean', 'tests/'+suite+'.lean'])
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullPeriodicEndAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 265, (len(closures), text)
...
report = dict(
    ...
    scope='Smooth period charts with matching end collars from local smooth real-base charts and a given smooth deck transformation; inverse seam and local smooth invertibility proved. Smooth pullback/deck data for arbitrary circle bundles and all-real smooth extension remain unproved.',
)
