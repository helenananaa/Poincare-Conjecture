#!/usr/bin/env python3
"""Validate recovered smooth atlas proofs plus relative open-interval cylinder classification."""
import argparse
import fcntl
import hashlib
import json
...
out = (a.output or package / '.lake/relative-product-audit').resolve()
...
suites = [
    ...
    'AbstractBundleCutSanity', 'ClosedRealBridgeSanity', 'AbstractBoundaryPairingSanity', 'SmoothSpliceSanity', 'PeriodicEndMatchingSanity', 'IntegerExtensionSanity', 'CoverPullbackSanity', 'SmoothAtlasPullbackSanity', 'RelativeProductSanity',
]
for suite in suites:
    run(suite, ['lake', 'env', 'lean', 'tests/'+suite+'.lean'])
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullRelativeProductAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 351, (len(closures), text)
...
report = dict(
    ...
    scope='Recovered smooth circle-bundle classification and new relative product classification for any open real interval. Original smooth structures and relative frontiers retained. 348 recovered plus 3 new audited declarations. No complete Poincare proof or independent review.',
)
