#!/usr/bin/env python3
"""Validate the original fiber proof plus the closed-collar component extension."""
...
repo = package.parent
out = (args.output or package/'.lake/closed-collar-audit').resolve()
...
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullFiberCollarAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 31, (len(closures), text)
...
report = dict(status='PASS', project_build_present_at_start=project_build_present_at_start, utc=datetime.now(timezone.utc).isoformat(),
              ...
              source_sha256={str(p.relative_to(package)):hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in sources},
              scope='Primary PoincareConjecture package; one completed blueprint node, plus verified closed-collar component and ambient-boundary interfaces; closed-collar blueprint not marked complete')
