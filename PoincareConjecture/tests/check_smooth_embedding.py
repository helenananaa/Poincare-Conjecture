...
repo = package.parent
out = (args.output or package/'.lake/smooth-embedding-audit').resolve()
...
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullSmoothEmbeddingAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 115, (len(closures), text)
...
report = dict(status='PASS', project_build_present_at_start=project_build_present_at_start, utc=datetime.now(timezone.utc).isoformat(),
              ...
              source_sha256={str(p.relative_to(package)):hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in sources},
              contribution_parent='929fc4962bf7e9a8486baebdf3bfa28a518d7540',
              scope='Equal-dimensional smooth embedding into a boundaryless ambient model: local charts constructed; intrinsic/ambient boundary proved; quotient adapter without extra chart or regular-open input; no smooth cylinder classification or full Poincare proof')
