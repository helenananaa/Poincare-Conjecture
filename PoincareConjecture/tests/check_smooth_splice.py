...
out = (a.output or package / '.lake/smooth-splice-audit').resolve()
...
expected_parent = 'aeb9ef1661669ffc4927d95f9c62fb1ae5924ce5'
...
suites = [
    ...
    'AbstractBundleCutSanity', 'ClosedRealBridgeSanity', 'AbstractBoundaryPairingSanity', 'SmoothSpliceSanity',
]
for suite in suites:
    run(suite, ['lake', 'env', 'lean', 'tests/'+suite+'.lean'])
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullSmoothSpliceAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 250, (len(closures), text)
...
report = dict(
    ...
    scope='Smooth matching and splicing of genuine real-base bundle charts; smooth interval trivialization from local smooth charts. No smooth periodic seam construction or complete Poincare proof.',
)
