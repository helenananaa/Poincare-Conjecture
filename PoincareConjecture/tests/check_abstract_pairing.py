...
out = (a.output or package / '.lake/abstract-pairing-audit').resolve()
...
suites = [
    ...
    'AbstractBundleCutSanity', 'ClosedRealBridgeSanity', 'AbstractBoundaryPairingSanity',
]
for suite in suites:
    run(suite, ['lake', 'env', 'lean', 'tests/'+suite+'.lean'])
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullAbstractPairingAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 227, (len(closures), text)
...
report = dict(
    ...
    scope='Closed/real mapping-torus comparison, bundle-derived continuous real presentation, '
          'and topological abstract sphere-bundle closure plus two actual frontier components. '
