#!/usr/bin/env python3
"""Validate smooth cylinder normalization and fixed-structure region diffeomorphisms."""
...
repo = package.parent
out = (args.output or package/'.lake/smooth-cylinder-audit').resolve()
...
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullSmoothCylinderAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 159, (len(closures), text)
...
report = dict(status='PASS', project_build_present_at_start=project_build_present_at_start, utc=datetime.now(timezone.utc).isoformat(),
              ...
              source_sha256={str(p.relative_to(package)):hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in sources},
              contribution_parent='b7e0d78bd82dcb2f3e8ada8fcca15bc9c81b15d5',
              scope='Standard cylinder diffeomorphisms, product-region classification, and periodic regions under explicit locally diffeomorphic presentation; no abstract bundle construction or complete Poincare proof')
