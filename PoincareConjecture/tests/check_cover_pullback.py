...
out = (a.output or package / '.lake/cover-pullback-audit').resolve()
...
expected_parent = '01093642d2c53898779ba3bd62a7ca8ac784a069'
...
suites = [
    ...
    'AbstractBundleCutSanity', 'ClosedRealBridgeSanity', 'AbstractBoundaryPairingSanity', 'SmoothSpliceSanity', 'PeriodicEndMatchingSanity', 'IntegerExtensionSanity', 'CoverPullbackSanity',
]
for suite in suites:
    run(suite, ['lake', 'env', 'lean', 'tests/'+suite+'.lean'])
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullCoverPullbackAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 328, (len(closures), text)
...
report = dict(
    ...
    scope='Canonical smooth structures on covering pullbacks, local diffeomorphic forgetful maps, and constructed smooth natural deck shifts. Smooth compatibility of pulled-back fiber trivializations is not asserted.',
)
