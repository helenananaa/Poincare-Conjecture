#!/usr/bin/env python3
"""Validate equal-dimensional smooth embedding boundary correspondence and its adapters."""
...
repo = package.parent
out = (args.output or package/'.lake/presentation-audit').resolve()
...
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullPresentationAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 142, (len(closures), text)
...
report = dict(status='PASS', project_build_present_at_start=project_build_present_at_start, utc=datetime.now(timezone.utc).isoformat(),
              ...
              source_sha256={str(p.relative_to(package)):hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in sources},
              contribution_parent='ead5275cb109e93584744a28497fa247a2382cca',
              scope='Comparison of a supplied continuous periodic sphere presentation with the concrete deck quotient; openness derived from compact fibers; smooth-closure theorem needs no given quotient homeomorphism; existence of arbitrary bundle presentations and smooth descent not proved')
