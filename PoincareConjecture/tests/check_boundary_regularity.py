#!/usr/bin/env python3
"""Validate the cumulative proofs and the conditional intrinsic boundary bridge."""
...
repo = package.parent
out = (args.output or package/'.lake/boundary-regularity-audit').resolve()
...
text = run('axioms', ['lake', 'env', 'lean', 'tests/FullBoundaryRegularityAudit.lean'])
...
for name in re.findall(r"'([^']+)' does not depend on any axioms", text):
    closures[name] = []
assert len(closures) == 100, (len(closures), text)
...
report = dict(status='PASS', project_build_present_at_start=project_build_present_at_start, utc=datetime.now(timezone.utc).isoformat(),
              ...
              source_sha256={str(p.relative_to(package)):hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in sources},
              contribution_parent='bcfed18b24464d7457d4a3fb52ca40e42b44f272',
              scope='Conditional intrinsic/ambient boundary bridge from explicit compatible chart extensions; no general smooth-embedding extension theorem or full Poincare proof')
