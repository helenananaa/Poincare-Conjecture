...
import PoincareConjecture.ParallelImplementation.SmoothRadiusControlledExtension
set_option autoImplicit false
noncomputable section
namespace PoincareConjecture.ParallelImplementation.SmoothRadiusControlledClassicalDeTurck
...
theorem exists_smooth_radius_controlled_classical_deturck
    (alpha : ℝ) (ha : 0 < alpha) (ha1 : alpha < 1)
    (u : E3 → E6) (hu : ContDiff ℝ ∞ u) (p0 : E3) (hp0 : u p0=0)
    (rmax : ℝ) (hrmax : 0 < rmax) :
...
by
  ...
  obtain ⟨r, hr, hr1, hrmax', U, hU, hUcompact, hUinner, hUouter,
      hU0, hU1, hU2, hUbound3⟩ :=
    PoincareConjecture.ParallelImplementation.SmoothRadiusControlledExtension.exists_smooth_radius_controlled_extension
      u hu p0 hp0 epsilon rmax hepsilon hrmax
  have hT : 0 < delta := hdelta
  have hUcont3 : ContDiff ℝ 3 U :=
    hU.of_le
      (WithTop.coe_le_coe.mpr (show (3 : ℕ∞) ≤ (⊤ : ℕ∞) from le_top))
  obtain ⟨z, hz, hzgraph, hzinc, hzbound, hzinitial, hzslices,
      hzmetric, hzequation⟩ :=
    hsolution delta hT le_rfl U hUcont3 hU0 hU1 hU2 hUbound3
...
/- SWARM_PROOF_END -/
end PoincareConjecture.ParallelImplementation.SmoothRadiusControlledClassicalDeTurck
