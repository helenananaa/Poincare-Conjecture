...
namespace PoincareConjecture.ParallelImplementation.FiniteRealizationCoordinates
open PoincareConjecture.ParallelImplementation.FinitePLRealization
open scoped BigOperators
/-- Exact coordinate characterization of the existing geometric realization. -/
theorem mem_realization_iff_coordinates
    {V : Type*} [Fintype V] [DecidableEq V]
    (K : FiniteAbstractComplex V) (x : V → ℝ) :
    x ∈ (realization K).space ↔
      (∀ v, 0 ≤ x v) ∧ (∑ v : V, x v) = 1 ∧
        (Finset.univ.filter fun v : V => 0 < x v) ∈ K.faces :=
/- SWARM_PROOF_BEGIN -/
by
  classical
  constructor
  · intro hx
    ...
    have hsub : S ⊆ s := by
      ...
      have hex : ∃ i, 0 < w i * (barycentricVertex (f i)) v :=
        (Pi.lt_def.mp hpos).2
...
/- SWARM_PROOF_END -/
end PoincareConjecture.ParallelImplementation.FiniteRealizationCoordinates
