import PoincareConjecture.ParallelImplementation.FullJetSpatialC3Slices
...
import PoincareConjecture.ParallelImplementation.SmallSmoothUniformHeatSolution
...
namespace PoincareConjecture.ParallelImplementation.SmallSmoothC3ClassicalDeTurck
...
theorem exists_small_smooth_c3_classical_deturck_solution
...
by
  ...
  rcases PoincareConjecture.ParallelImplementation.ConcreteSixGeometricRHS.exists_concrete_six_geometric_rhs with
    ⟨B, hBnorm, hBidentity⟩
  rcases PoincareConjecture.ParallelImplementation.SmallSmoothUniformHeatSolution.exists_small_smooth_unforced_uniform_heat_solution
      alpha ha ha1 B with
  ...
  obtain ⟨z, hz, hztimeGraph, hzinc, hzbound, hzinitial, hUniform, b, hbgraph, hbdata⟩ :=
  ...
  obtain ⟨Cdiff, hCdiff, hquot⟩ := hUniform
  have hC3 := FullJetSpatialC3Slices.spatial_slices_contDiff_three_of_full_jet_differences
    T alpha hT.le ha ha1.le z hz hztimeGraph hzinc Cdiff hCdiff.le hquot
  have hC1 := FullJetJointC1.contDiffOn_one_of_actual_full_jet T alpha hT z hz hztimeGraph
  refine ⟨z, hz, hztimeGraph, hzinc, hzbound, hzinitial, hC3, hC1,
    ⟨Cdiff, hCdiff, hquot⟩, ?_, ?_⟩
  · intro p
  ...
  · intro t ht0 htT x i j
    ...
    have hpde : z.1.2 p - traceVec =
      ...
      have h := hdata.2.2
      rw [hhess, hgrad] at h
      simpa [F, traceVec, correctionVec] using h
    ...
    have hmetricDeriv := hmetricDeriv i j
    rw [hcoordinate] at hmetricDeriv
    have hforcing : symmetricSixMatrix (F.1 p) i j = 0 := by
      change symmetricSixMatrix (0 : E6) i j = 0
      exact hsymZero i j
    rw [hforcing] at hmetricDeriv
    simpa [p, u] using hmetricDeriv
/- SWARM_PROOF_END -/
end PoincareConjecture.ParallelImplementation.SmallSmoothC3ClassicalDeTurck
