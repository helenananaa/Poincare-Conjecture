...
by
  ...
  have hshift : ContDiff ℝ 3 (fun x : E3 => p + A x) := by
    fun_prop
  have hMshift : ContDiff ℝ 3 (fun x : E3 => M (p + A x)) :=
    hM.comp hshift
  have hCshift : ContDiff ℝ 3 (fun x : E3 => C (M (p + A x))) := by
    exact ContDiff.continuousLinearMap_comp C hMshift
  have hN : ContDiff ℝ 3 N := by
    dsimp [N]
    exact hCshift.sub contDiff_const
  have hu : ContDiff ℝ 3 u := by
    dsimp [u]
...
/- SWARM_PROOF_END -/
end PoincareConjecture.ParallelImplementation.NormalizedSixMetricCoordinates
