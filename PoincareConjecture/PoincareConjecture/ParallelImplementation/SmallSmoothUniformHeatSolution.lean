import PoincareConjecture.ParallelImplementation.UniformAffineRationalDifference
...
namespace PoincareConjecture.ParallelImplementation.SmallSmoothUniformHeatSolution
...
theorem exists_small_smooth_unforced_uniform_heat_solution
    ...
    ∃ E : E6 →L[ℝ] (E3 →L[ℝ] E3), Function.Injective E ∧ ‖E‖ ≤ 3 ∧
      ...
      ∃ epsilon delta C : ℝ, 0 < epsilon ∧ 0 < delta ∧ delta ≤ 1 ∧ 0 < C ∧
        ...
        ∀ (u0 : E3 →ᵇ E6) (A0 : E3 →ᵇ (E3 →L[ℝ] E6))
          ...
          ContDiff ℝ ∞ (u0 : E3 → E6) → HasCompactSupport (u0 : E3 → E6) →
          ∃ z : FullJet T, z.1.1 ∈ parabolicC2HolderSet T alpha ∧
            ...
            (∃ Cdiff : ℝ, 0 < Cdiff ∧ ∀ (i : Fin 3) (h : ℝ), h ≠ 0 →
              ‖PoincareConjecture.ParallelImplementation.FiniteSpatialDifferenceJet.finiteSpatialDifferenceJet z i h‖ ≤ Cdiff) ∧
            ∃ b : ForcingJet (E3 →L[ℝ] E3) T, b ∈ forcingGraph (E3 →L[ℝ] E3) T alpha ∧
              ∀ p : Slab T, ((1+E (z.1.1.1.1 p))*b.1 p=1 ∧ b.1 p*(1+E (z.1.1.1.1 p))=1) ∧
                ...
                z.1.2 p-(∑ i : Fin 3, z.1.1.1.2.2 p (EuclideanSpace.single i 1) (EuclideanSpace.single i 1)) =
                  (0 : ForcingJet E6 T).1 p+(∑ i : Fin 3, ∑ j : Fin 3, ((b.1 p-1) (EuclideanSpace.single j 1)) i •
...
by
  ...
  intro T hT hTdelta u0 A0 H0 hu0 hA0 hu0norm hA0norm hH0norm
    hu0holder hA0holder hH0holder huSmooth huCompact
  let F : ForcingJet E6 T := 0
  have hFgraph : F ∈ forcingGraph E6 T alpha := by intro p; simp [F]
  ...
  have htotalHeat (p : Slab T) :
    ...
    rw [hyHeat p]
    abel
  have hUniformDifference : ∃ Cdiff : ℝ, 0 < Cdiff ∧ ∀ (i : Fin 3) (h : ℝ), h ≠ 0 →
      ‖PoincareConjecture.ParallelImplementation.FiniteSpatialDifferenceJet.finiteSpatialDifferenceJet z i h‖ ≤ Cdiff := by
    have hA0Value (x : E3) : A0 x = fderiv ℝ (u0 : E3 → E6) x := by
      apply (hu0 x).unique
      exact (huSmooth.differentiable (by norm_num) x).hasFDerivAt
    have hA0Function : (A0 : E3 → (E3 →L[ℝ] E6)) =
        fderiv ℝ (u0 : E3 → E6) := funext hA0Value
    have hH0Value (x : E3) : H0 x =
        fderiv ℝ (fderiv ℝ (u0 : E3 → E6)) x := by
      apply (hA0 x).unique
      have hA0diff : DifferentiableAt ℝ (A0 : E3 → (E3 →L[ℝ] E6)) x := by
        rw [hA0Function]
        exact (((huSmooth.fderiv_right (m := 1)
        (show (2 : ℕ∞ω) ≤ ∞ from
          WithTop.coe_le_coe.mpr (show (2 : ℕ∞) ≤ ⊤ from le_top))).differentiable
        (by norm_num)) x)
      have hA0derivValue : fderiv ℝ (A0 : E3 → (E3 →L[ℝ] E6)) x =
          fderiv ℝ (fderiv ℝ (u0 : E3 → E6)) x := by
        rw [hA0Function]
      rw [← hA0derivValue]
      exact hA0diff.hasFDerivAt
    have hvValue' (p : Slab T) : v0.1 p =
        fderiv ℝ (u0 : E3 → E6) p.2 := by
      calc
        v0.1 p = s.1.1.1.2.1 p := hv0Spatial p
        _ = A0 p.2 := (hsData p).2.1
        _ = fderiv ℝ (u0 : E3 → E6) p.2 := hA0Value p.2
    have hHValue' (p : Slab T) : Hbg.1 p =
        fderiv ℝ (fderiv ℝ (u0 : E3 → E6)) p.2 := by
      calc
        Hbg.1 p = s.1.1.1.2.2 p := hHbgSpatial p
        _ = H0 p.2 := (hsData p).2.2.1
        _ = fderiv ℝ (fderiv ℝ (u0 : E3 → E6)) p.2 := hH0Value p.2
    have haValue' (p : Slab T) : a.1 p = -E (Fu.1 p) := by
      calc
        a.1 p = -E (s.1.1.1.1 p) := haValue p
        _ = -E (u0 p.2) := by rw [(hsData p).1]
        _ = -E (Fu.1 p) := by rw [hFuValue p]
    have hfValue' (p : Slab T) : f.1.1 p =
        ∑ j : Fin 3, Hbg.1 p (e j) (e j) := by
      change (F + traceH Hbg).1 p = _
      rw [show F = (0 : ForcingJet E6 T) by rfl]
      simp only [zero_add]
      rw [htraceHValue Hbg p]
      simp [tr, e]
    have hUniformEquation (p : Slab T) :
        ((1 - (a + P y).1 p) * b.1 p = 1 ∧
          b.1 p * (1 - (a + P y).1 p) = 1) ∧
        (L y).1.1 p = f.1.1 p +
          PoincareConjecture.ParallelImplementation.RationalSpatialDifferenceQuotient.principalPart
            (b.1 p - 1) ((Hbg + Hproj y).1 p) +
          B (b.1 p) (b.1 p) ((v0 + Q y).1 p) ((v0 + Q y).1 p) := by
      refine ⟨hbInverse p, ?_⟩
      simpa [PoincareConjecture.ParallelImplementation.RationalSpatialDifferenceQuotient.principalPart,
        ...
        using hCoupled p
    obtain ⟨Cdiff, hCdiff, hDifferenceBounds⟩ :=
      PoincareConjecture.ParallelImplementation.UniformAffineRationalDifference.uniform_affine_rational_difference_bounds
        T alpha hT X Y hX hY E B L P Q Hproj
        hPgraph hQgraph hHgraph'
        hPvalue hQvalue hHvalue hLvalue
        a v0 Hbg f y b hbgraph hUniformEquation
        D hLD R hR hy
        haGraph hv0Graph hHbgGraph hsmallCoupled hDcontract
        (u0 : E3 → E6) huSmooth huCompact ha ha1.le hEnorm
        Fu hFuGraph hFuValue haValue' hvValue' hHValue' hfValue'
    have hsStationaryData (p : Slab T) :
        s.1.1.1.1 p = (u0 : E3 → E6) p.2 ∧
          s.1.1.1.2.1 p = fderiv ℝ (u0 : E3 → E6) p.2 ∧
          s.1.1.1.2.2 p = fderiv ℝ (fderiv ℝ (u0 : E3 → E6)) p.2 ∧
          s.1.2 p = 0 := by
      rcases hsData p with ⟨hsu, hsA, hsH, hst⟩
      exact ⟨hsu,
        calc s.1.1.1.2.1 p = A0 p.2 := hsA
             _ = fderiv ℝ (u0 : E3 → E6) p.2 := hA0Value p.2,
        calc s.1.1.1.2.2 p = H0 p.2 := hsH
             _ = fderiv ℝ (fderiv ℝ (u0 : E3 → E6)) p.2 := hH0Value p.2,
        hst⟩
    refine ⟨Cdiff, hCdiff, ?_⟩
    intro i h hh
    exact (hDifferenceBounds i h hh).2 s hsStationaryData hsSpace.2 hsInc
  refine ⟨z, hzSpace, hzTime, hzInc, hzNorm, hzInitial, hUniformDifference, b, hbgraph, ?_⟩
  intro p
  refine ⟨?_, ?_⟩
  · refine ⟨?_, ?_⟩
    · rw [← hcoeff p]
...
/- SWARM_PROOF_END -/
end PoincareConjecture.ParallelImplementation.SmallSmoothUniformHeatSolution
