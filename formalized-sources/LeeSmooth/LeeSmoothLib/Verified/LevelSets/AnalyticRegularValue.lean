...
/-!
Genuinely analytic regular-level-set theorem.

The construction is the analytic inverse-function theorem, producing charts in the outer-top
(`⊤`) maximal atlas, followed by analytic Euclidean slice charts and Theorem 5.8's analytic
atlas packaging.  It does not promote a `C∞` atlas to an analytic one, and it does not use the
sorry-filled Theorem 5.12 / Corollary 5.14 owners.
-/
...
namespace LeeVerifiedAnalyticLevelSets

/-! ### Analytic IFT local normal form on Euclidean models -/
...
/-- Euclidean-model analytic regular-level theorem.  Charts, manifold structure and the
embedded-submanifold owner are all at outer top.  The bound `n ≤ m` fixes the fibre
dimension even when the fibre is empty. -/
theorem euclidean_analytic_regular_level_set_has_embedded_submanifold_structure
    [T2Space M] [SecondCountableTopology M]
    {Φ : M → N} {c : N}
    (hΦ : ContMDiff I_m I_n (⊤ : WithTop ℕ∞) Φ)
    (hc : Manifold.IsRegularValue I_m I_n Φ c)
    (hnm : n ≤ m) :
    let k : ℕ := m - n
    let S : Set M := Φ ⁻¹' {c}
    ∃ cs : ChartedSpace (EuclideanSpace ℝ (Fin k)) S,
      ∃ hs : IsManifold (𝓡 k) (⊤ : WithTop ℕ∞) S,
        ...
        ∃ hS : IsEmbeddedSubmanifold I_m (𝓡 k) S,
          hS.codimension = n := by
  dsimp
  let S : Set M := Φ ⁻¹' {c}
  ...
  have hSlice : Set.SatisfiesLocalSliceCondition m S (m - n) := by
    ...
    rcases at_of_surjective_mfderiv hΦ p (hc p hpc) with ⟨h, _⟩
  ...
  refine ⟨cs, hs, ?_⟩
  letI : ChartedSpace (EuclideanSpace ℝ (Fin (m - n))) S := cs
  letI : IsManifold (𝓡 (m - n)) (⊤ : WithTop ℕ∞) S := hs
  refine ⟨hEmb, ?_⟩
  change Module.finrank ℝ (EuclideanSpace ℝ (Fin m)) -
      Module.finrank ℝ (EuclideanSpace ℝ (Fin (m - n))) = n
  have hm : Module.finrank ℝ (EuclideanSpace ℝ (Fin m)) = m := by simp
  have hk : Module.finrank ℝ (EuclideanSpace ℝ (Fin (m - n))) = m - n := by simp
  rw [hm, hk]
  exact Nat.sub_sub_self hnm

/-- Public name for the Euclidean analytic regular-level theorem.  Downstream self-model
users transport along `toEuclideanCLM` / `euclideanChartedSpaceSelf` first. -/
theorem analytic_regular_level_set_has_embedded_submanifold_structure
    ...
    ∃ cs : ChartedSpace (EuclideanSpace ℝ (Fin k)) S,
      ∃ hs : IsManifold (𝓡 k) (⊤ : WithTop ℕ∞) S,
        ...
        ∃ hS : IsEmbeddedSubmanifold I_m (𝓡 k) S,
          hS.codimension = n :=
  euclidean_analytic_regular_level_set_has_embedded_submanifold_structure hΦ hc hnm

/-- Corollary: an analytic regular level set is closed, hence properly embedded as a subset. -/
theorem euclidean_analytic_regular_level_set_isProperlyEmbedded
    [T1Space N] {Φ : M → N} {c : N}
    (hΦ : ContMDiff I_m I_n (⊤ : WithTop ℕ∞) Φ)
    (_hc : Manifold.IsRegularValue I_m I_n Φ c) :
    (Φ ⁻¹' {c}).IsProperlyEmbedded :=
  (isClosed_singleton.preimage hΦ.continuous).isProperlyEmbedded

end EuclideanModels


/-! ### Model transport: self-models `𝓘(ℝ, E)` to Euclidean charts

A finite-dimensional real vector space is linearly equivalent to a Euclidean space.
Postcomposing the self-model atlas by that equivalence yields an analytic Euclidean atlas,
because linear maps are analytic.  This is the bridge used by matrix groups.
-/

universe uE uE' uM' uN'

section ModelTransport

variable {E : Type uE} [NormedAddCommGroup E] [NormedSpace ℝ E] [FiniteDimensional ℝ E]
variable {E' : Type uE'} [NormedAddCommGroup E'] [NormedSpace ℝ E'] [FiniteDimensional ℝ E']
variable {M : Type uM'} [TopologicalSpace M] [ChartedSpace E M]
  [IsManifold 𝓘(ℝ, E) (⊤ : WithTop ℕ∞) M]
variable {N : Type uN'} [TopologicalSpace N] [ChartedSpace E' N]
  [IsManifold 𝓘(ℝ, E') (⊤ : WithTop ℕ∞) N]
...
theorem euclideanChartedSpaceSelf_isManifold
    {F P : Type*} [NormedAddCommGroup F] [NormedSpace ℝ F] [FiniteDimensional ℝ F]
    [TopologicalSpace P] [ChartedSpace F P]
    [IsManifold 𝓘(ℝ, F) (⊤ : WithTop ℕ∞) P] :
    letI := euclideanChartedSpaceSelf (P := P) (F := F)
    IsManifold (𝓡 (Module.finrank ℝ F)) (⊤ : WithTop ℕ∞) P := by
  letI := euclideanChartedSpaceSelf (P := P) (F := F)
  refine isManifold_of_contDiffOn (𝓡 (Module.finrank ℝ F)) (⊤ : WithTop ℕ∞) P ?_
  intro e e' he he'
  rcases he with ⟨χ, hχ, rfl⟩
  rcases he' with ⟨χ', hχ', rfl⟩
  let eE := toEuclideanCLM F
  have horigMem :
      χ.symm.trans χ' ∈ contDiffGroupoid (⊤ : WithTop ℕ∞) 𝓘(ℝ, F) :=
  ...
  have horig' :
      ContDiffOn ℝ (⊤ : WithTop ℕ∞) (↑(χ.symm.trans χ'))
...
/-- An analytic regular level set is closed, hence properly embedded as a subset. -/
theorem analytic_regular_level_set_isProperlyEmbedded
    [T1Space N] {Φ : M → N} {c : N}
    (hΦ : ContMDiff 𝓘(ℝ, E) 𝓘(ℝ, E') (⊤ : WithTop ℕ∞) Φ)
    (_hc : Manifold.IsRegularValue 𝓘(ℝ, E) 𝓘(ℝ, E') Φ c) :
    (Φ ⁻¹' {c}).IsProperlyEmbedded :=
  (isClosed_singleton.preimage hΦ.continuous).isProperlyEmbedded

end ModelTransport

end LeeVerifiedAnalyticLevelSets
