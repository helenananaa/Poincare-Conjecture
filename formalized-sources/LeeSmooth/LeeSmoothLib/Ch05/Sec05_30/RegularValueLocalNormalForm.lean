...
/-!
Direct `C∞` local normal form at a regular point.  The construction complements the
surjective derivative and applies the inverse function theorem.  It does not use the
...
namespace RegularValueLocalNormalForm
...
/-- At a point where the derivative is surjective, a smooth map has the projection normal form.
...
theorem at_of_surjective_mfderiv
    {F : M → N} (hF : ContMDiff I_m I_n ∞ F) (p : M)
  ...
  have hΨzero : Ψ 0 = 0 := by
    rw [← hΨeq h0Ψ, hGzero]
  let domCenter : E_m ≃ₘ[ℝ] (X × Y) := centeredLinearDiffeomorph d a
  let domFinish : (X × Y) ≃ₘ[ℝ] E_m := (finSplit hrm).symm.toDiffeomorph
  let domP : PartialDiffeomorph I_m I_m M E_m ∞ :=
  ...
  let codCenter : E_n ≃ₘ[ℝ] E_n := centeredLinearDiffeomorph codLinear b
  let codP : PartialDiffeomorph I_n I_n N E_n ∞ :=
  ...
  have hcodZero : codChart (F p) = 0 := by
    ...
    simp [b]
  have hdomMax : domChart ∈ IsManifold.maximalAtlas I_m ∞ M :=
    domChart.mem_maximalAtlas_of_contMDiffOn
      domP.contMDiffOn_toFun domP.contMDiffOn_invFun
  have hcodMax : codChart ∈ IsManifold.maximalAtlas I_n ∞ N :=
...
end RegularValueLocalNormalForm
