/-
Adapted from Kai Lam's mathlib PR 36770, commit
230d75acb32d80e7d7c4f4cd028b139f3dc28be7. Apache-2.0.
Only the invariance-of-domain argument is retained; unrelated unfinished retraction proofs
are not imported. The fixed-point hypothesis must be discharged by the genuine Brouwer proof.
-/
import Mathlib
...
class BrouwerFixedPoint (E : Type*) [NormedAddCommGroup E] : Prop where
...
theorem differentiable_approx_of_continuous {δ : ℝ} (hδ : 0 < δ) {U : Set E}
  ...
  have hP_bound : ∀ y ∈ U , ‖P y - G y‖ < δ := by
    ...
    have hP_eq : P y - G y = coordEquiv.symm v := by
      change coordEquiv.symm (fun i => (p_i i : C(E, ℝ)) y) - G y = coordEquiv.symm v
      rw [show G y = coordEquiv.symm (coordEquiv (G y)) by
        exact (coordEquiv.symm_apply_apply (G y)).symm]
      rw [← coordEquiv.symm.map_sub]
      rfl
    rw [hP_eq]
    calc
      ‖coordEquiv.symm v‖
...
theorem invariance_of_domain_interior (f : E → E)
  ...
  have hsigma1compact : IsCompact sigma1 := by
    ...
    have hset : {y | ‖y - c‖ ≥ ε }ᶜ = ball c ε := by
      ext y
  ...
  have hPhicont : ContinuousOn Phi (f '' closedBall 0 1) := by
    ...
    (by
      ...
      have hyc : (y : E) ≠ c := by
        intro h
        exact hc2 (h ▸ y.property)
      exact norm_ne_zero_iff.mpr (sub_ne_zero.mpr hyc))) continuous_const
  ...
  have hpeturb_bound : ∀ y ∈ f '' (closedBall (0 : E) 1), ‖G y - Gtilde y‖ ≤ 1 := by
    ...
    · -- If `‖y - c‖ ≤ ε`, then `Φ y ∈ Σ₂`.
      ...
      have hP_approx_le : ‖P (Phi y)‖ ≤ ‖(G (Phi y) : E)‖ + δ := by
        calc
          ‖P (Phi y)‖ ≤ ‖(G (Phi y) : E)‖ + ‖P (Phi y) - G (Phi y)‖ :=
            norm_le_norm_add_norm_sub' _ _
          _ ≤ ‖(G (Phi y) : E)‖ + δ := by
            have h := le_of_lt (hP_bound (Phi y) (Or.inr hyimg))
            exact add_le_add_right h ‖(G (Phi y) : E)‖
      have hG_phi_small : ‖(G (Phi y) : E)‖ ≤ 0.1 := by
        ...
        have hsmall := h2ε1 hdist
        rw [hG0, dist_zero_right] at hsmall
