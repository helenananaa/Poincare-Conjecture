import Mathlib
import LeeSmoothLib.External.SardMoreira.ToMathlib.ContinuousLinearMap
...
theorem compAlongOrderedFinpartition_sub_compAlongOrderedFinpartition_isBigO
  ...
  · simp only [← isBigO_one_iff ℝ, ← isBigO_pi] at *
    ...
    rcases (hp_bdd.norm_left.mul (H.const_mul_left c.length)).bound with ⟨C, hC⟩
    apply IsBigO.of_bound C
    filter_upwards [hC] with x hx
    have hq₁ : (fun m ↦ q₁ x m) = q₁ x := rfl
    have hq₂ : (fun m ↦ q₂ x m) = q₂ x := rfl
    have hqsub : (fun m ↦ q₁ x m) - (fun m ↦ q₂ x m) = fun m ↦ q₁ x m - q₂ x m := by
      ext
      rfl
    rw [hq₁, hq₂, hqsub]
...
theorem OpenPartialHomeomorph.iteratedFDeriv_symm_eq_rec [CompleteSpace E]
  ...
  · specialize hf' hi₀
    ...
    ext v
    simp +unfoldPartialApp [OrderedFinpartition.applyOrderedFinpartition,
      OrderedFinpartition.atomic_length, OrderedFinpartition.atomic_partSize, ftaylorSeries,
      hf'.fderiv, (f.hasFDerivAt_symm hy hf').fderiv, Function.comp_def]
