...
import Mathlib
import LeeSmoothLib.External.SardMoreira.UnifDoublingCover
...
theorem EMetric.diam_metricClosedBall_le {X : Type*} [PseudoMetricSpace X]
  ...
  · simp [ENNReal.ofReal_of_nonpos hr.le, Metric.closedBall_of_neg hr, Metric.ediam_empty]
  lift r to ℝ≥0 using hr
  rw [← Metric.closedEBall_coe]
  simpa using (Metric.ediam_closedEBall_le (x := x) (r := (r : ℝ≥0∞)))
...
instance instClosedBallCoveringMeasureOfIsUnifLocDoublingMeasure [BorelSpace α] [SecondCountableTopology α]
  ...
  exists_closedBall_covering_tsum_measure_le := by
    ...
    set t : Set (α × ℝ) :=
      {(c, r) : α × ℝ | c ∈ s ∧
        r ∈ f c ∩ Ioo 0 (IsUnifLocDoublingMeasure.scalingScaleOf μ 3) ∧ closedBall c r ⊆ U}
    obtain ⟨u, hus, huc, hud, hμ⟩ : ∃ u ⊆ t, u.Countable ∧
        u.PairwiseDisjoint (fun a ↦ closedBall a.1 a.2) ∧
        μ (s \ ⋃ a ∈ u, closedBall a.1 a.2) = 0 := by
      apply Vitali.exists_disjoint_covering_ae μ s t
        (IsUnifLocDoublingMeasure.scalingConstantOf μ 3) Prod.snd Prod.fst
      · exact fun _ _ ↦ Subset.rfl
      · exact fun x hx ↦ IsUnifLocDoublingMeasure.measure_mul_le_scalingConstantOf_mul _
          (by simp) hx.2.1.2.2.le
      · intro x hx
      ...
      · intro x hx ε hε
        ...
        rcases hf x hx (ε ⊓ δ ⊓ IsUnifLocDoublingMeasure.scalingScaleOf μ 3)
          (lt_min (lt_min hε hδ₀) (IsUnifLocDoublingMeasure.scalingScaleOf_pos _ _))
          with ⟨r, hrf, hr₀, hrε⟩
        rw [lt_min_iff, lt_min_iff] at hrε
        refine ⟨(x, r), ⟨hx, ⟨hrf, hr₀, hrε.2⟩, ?_⟩, hrε.1.1.le, rfl⟩
        grw [hrε.1.2, hδU]
    rcases IsUnifLocDoublingMeasure.exists_closedBall_covering_tsum_measure_le_of_measure_zero μ
...
lemma outerMeasure_null_of_forall_le_mul_ae_null {μ : Measure α} [SigmaFinite μ]
  ...
  · calc
      ν (s ∩ {x | C x = 0}) ≤ 0 * μ (s ∩ {x | C x = 0}) := by
        ...
        refine (h x hx.1).mono fun εr hεr ↦ ?_
        exact (ν.mono (inter_subset_inter_left _ inter_subset_left)).trans <| by
          simpa [hx.2.out] using hεr
      _ = 0 := zero_mul _
  · set t := s \ {x | C x = 0}
    have hμt : μ t = 0 := by
      rw [ae_iff] at hC
      rw [show t = {x | x ∈ s ∧ ¬ C x = 0} by ext x; simp [t]]
      simpa only [_root_.not_imp] using hC
    calc
