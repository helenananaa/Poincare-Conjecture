import LeeSmoothLib.External.SardMoreira.UpperLowerSemicontinuous
import LeeSmoothLib.External.SardMoreira.NormedSpace
import LeeSmoothLib.External.SardMoreira.MeasureComap
import LeeSmoothLib.External.SardMoreira.MeasureBallSemicontinuous
import LeeSmoothLib.External.SardMoreira.Topology
    ...
    _ ≤ μ.comap (↑) ({(0 : E)}ᶜ ↓∩ s) := by
      ...
      · refine (setLIntegral_mono ?_ ?_).trans (setLIntegral_le_lintegral _ _)
        ...
        · intro x hx
          have hset :
              Prod.mk x ⁻¹'
                  (⇑(homeomorphUnitSphereProd E).toMeasurableEquiv.symm ⁻¹'
                    ({(0 : E)}ᶜ ↓∩ s)) =
                {t : Ioi (0 : ℝ) | (t : ℝ) • (x : E) ∈ s} := by
            ext t
