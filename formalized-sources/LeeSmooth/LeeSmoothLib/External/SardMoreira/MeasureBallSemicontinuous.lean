...
theorem MeasureTheory.tendsto_measure_biUnion_lt {α : Type*} {m : MeasurableSpace α}
  ...
  have : (atTop : Filter (Iio a)).IsCountablyGenerated := by
    ...
    infer_instance
  let t : Iio a → Set α := fun i ↦ s i
  have ht : Monotone t := fun i j hle ↦ hm i j hle j.2
  have H := tendsto_measure_iUnion_atTop (μ := μ) ht
  change Tendsto (fun i : Iio a ↦ (μ ∘ s) i) atTop (𝓝 (μ (⋃ i, t i))) at H
  rw [tendsto_comp_coe_Iio_atTop] at H
  convert H using 1 <;> simp [t]
