...
import LeeSmoothLib.External.SardMoreira.LocalEstimates
...
theorem fderiv_implicitFunction_chartImplicitData_apply_mk_zero {f : E × F → ℝ} {a : E × F}
  ...
  let ψ := chartImplicitData f a hfa hk hdf
  have hpt : ψ.pt = a := chartImplicitData_pt hfa hk hdf
  have hleft : ψ.leftFun ψ.pt = f a := by simp [ψ]
  have hright : ψ.rightFun ψ.pt = ψ.rightFun a := congr_arg ψ.rightFun hpt
  rw [← hleft, ← hright]
  apply ψ.fderiv_implicitFunction_apply_eq_iff.mpr
  constructor
  · cases y with | mk y hy => simpa [ψ] using hy
...
theorem exists_dim_lt_map_nhdsWithin_eq (hs : ¬IsLargeAt k α s a)
  ...
  have hleftFun : ψ.leftFun = f := by simp [ψ]
  have hleft : ψ.leftFun ψ.pt = 0 := by simp [ψ, hfa₀]
  have hga : g (ψ.rightFun a) = a := by
    calc
      g (ψ.rightFun a) = ψ.implicitFunction 0 (ψ.rightFun a) := rfl
      _ = ψ.implicitFunction (ψ.leftFun ψ.pt) (ψ.rightFun ψ.pt) := by rw [hleft, hpt]
      _ = ψ.pt := ψ.implicitFunction_apply_image.self_of_nhds
      _ = a := hpt
  have hg_tendsto : Tendsto g (𝓝 (ψ.rightFun a)) (𝓝 a) := by
    have H := ψ.differentiableAt_implicitFunction.continuousAt.tendsto
    have hga' : ψ.implicitFunction 0 (ψ.rightFun a) = a := hga
    rw [hleft, hpt, hga'] at H
    exact H
  ...
  have HcontDiff : ∀ᶠ x in 𝓝 (ψ.rightFun a), (g x ∈ s → ContDiffMoreiraHolderAt k α g x) := by
    ...
    suffices ContDiffMoreiraHolderAt k α ψ.toOpenPartialHomeomorph.symm (0, x) from
      by
        apply (this.comp (.prodMk .const .id) hk).congr_eventuallyEq
        exact .of_forall fun z ↦ ψ.implicitFunction_apply.symm
    apply OpenPartialHomeomorph.contDiffMoreiraHolderAt_symm _ hx₁ hx₂
    have hright : ContDiffMoreiraHolderAt k α ψ.rightFun (g x) := by
      simp only [ψ, chartImplicitData]
      apply ContinuousLinearMap.contDiffMoreiraHolderAt
    have hprod := (hx₃ hgx).prodMk hright
    have hpoint : ψ.toOpenPartialHomeomorph.symm (0, x) = g x :=
      ψ.implicitFunction_apply.symm
    rw [hpoint]
    apply hprod.congr_eventuallyEq
    exact .of_forall fun z ↦ by
      rw [ψ.toOpenPartialHomeomorph_apply, hleftFun]
...
theorem nonempty_atlas {k : ℕ} (hk : k ≠ 0) (α : I) (s : Set (E × F)) :
  ...
  · left
    simp only [Chart.id_toFun, Chart.id_set, image_id, mem_setOf_eq]
    exact ⟨hx, hxt⟩
  · right
