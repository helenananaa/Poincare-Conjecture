...
universe uE uE1 uE2 uH uH1 uH2 uM uN1 uN2

variable {E : Type uE} [NormedAddCommGroup E] [NormedSpace ℝ E] [FiniteDimensional ℝ E]
variable {E1 : Type uE1} [NormedAddCommGroup E1] [NormedSpace ℝ E1]
  [FiniteDimensional ℝ E1]
variable {E2 : Type uE2} [NormedAddCommGroup E2] [NormedSpace ℝ E2]
  [FiniteDimensional ℝ E2]
variable {H : Type uH} [TopologicalSpace H]
variable {H1 : Type uH1} [TopologicalSpace H1]
variable {H2 : Type uH2} [TopologicalSpace H2]
variable {I : ModelWithCorners ℝ E H}
variable {J1 : ModelWithCorners ℝ E1 H1}
variable {J2 : ModelWithCorners ℝ E2 H2}
...
/-- Theorem 4.31 (Uniqueness of Smooth Quotients): in the verified real finite-dimensional,
boundaryless-source setting, surjective smooth submersions from a common smooth manifold that are
constant on each other's fibers determine a unique diffeomorphism between their targets
intertwining the quotient maps. -/
theorem existsUnique_diffeomorph_of_surjective_smooth_submersions_constant_on_each_others_fibers
    ...
    ∃! F : N1 ≃ₘ⟮J1, J2⟯ N2, F ∘ pi1 = pi2 := by
  obtain ⟨F, ⟨hF_smooth, hF_comp⟩, -⟩ :=
    existsUnique_contMDiff_lift_of_surjective_smooth_submersion
      hpi1 hsurj1 hpi2.contMDiff hfib12
  obtain ⟨G, ⟨hG_smooth, hG_comp⟩, -⟩ :=
    existsUnique_contMDiff_lift_of_surjective_smooth_submersion
      hpi2 hsurj2 hpi1.contMDiff hfib21
  have hGF : Function.LeftInverse G F := by
    intro y
    obtain ⟨x, rfl⟩ := hsurj1 y
    calc
      G (F (pi1 x)) = G (pi2 x) := congrArg G (congrFun hF_comp x)
      _ = pi1 x := congrFun hG_comp x
  have hFG : Function.RightInverse G F := by
    intro y
    obtain ⟨x, rfl⟩ := hsurj2 y
    calc
      F (G (pi2 x)) = F (pi1 x) := congrArg F (congrFun hG_comp x)
      _ = pi2 x := congrFun hF_comp x
  let e : N1 ≃ N2 :=
    { toFun := F
      invFun := G
      left_inv := hGF
  ...
  · intro d' hd'
    ...
    ext y
    obtain ⟨x, rfl⟩ := hsurj1 y
    change d' (pi1 x) = F (pi1 x)
    calc
      d' (pi1 x) = pi2 x := congrFun hd' x
      _ = F (pi1 x) := (congrFun hF_comp x).symm
