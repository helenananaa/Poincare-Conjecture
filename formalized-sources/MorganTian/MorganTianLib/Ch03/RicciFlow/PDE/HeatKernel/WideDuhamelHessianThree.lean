...
/-- Time integration of the actual three-dimensional Hessian convolution. -/
theorem euclideanHeatKernel_three_duhamel_hessian_control (alpha : ℝ)
    (ha : 0 < alpha) (ha1 : alpha < 1) :
    ∃ C : ℝ, 0 < C ∧ ∀ (F : ℝ × E3 → ℝ), Continuous F →
      ∀ (B L T : ℝ), 0 ≤ B → 0 ≤ L → 0 ≤ T →
      (∀ s ∈ Icc (0:ℝ) T, ∀ x : E3, |F (s,x)| ≤ B) →
      ...
      ∀ (i j : Fin 3) (x : E3),
        ...
        |∫ s in (0:ℝ)..T, ∫ y : E3,
          fderiv ℝ (fun z : E3 => fderiv ℝ (euclideanHeatKernel 3 (T-s)) z
            (EuclideanSpace.single i 1)) y (EuclideanSpace.single j 1) * F (s,x-y)| ≤ C*L*T^(alpha/2) :=
/- SWARM_PROOF_BEGIN -/
by
  classical
  ...
  have hC : 0 < C := by
    ...
    positivity
  refine ⟨C, hC, ?_⟩
  intro F hF B L T hB hL hT hFbound hHolder i j x
  by_cases hT0 : T = 0
  · subst T
    simp
    positivity
  · have hTpos : 0 < T := lt_of_le_of_ne hT (Ne.symm hT0)
    let g : ℝ → ℝ := fun s =>
      ∫ y : E3, fderiv ℝ (fun z : E3 => fderiv ℝ (euclideanHeatKernel 3 (T-s)) z
    ...
    let g₀ : ℝ → ℝ := fun s =>
      ∫ y : E3, H (T-s) y * F (s,x-y)
    let w : ℝ → ℝ := fun s => C₀ * L * (T-s) ^ (alpha / 2 - 1)
    have hconvolution_eq : ∀ s ∈ Ioc (0 : ℝ) T, s ≠ T →
      ...
      exact ae_of_all _ (fun y => by
        change fderiv ℝ (fun z : E3 => fderiv ℝ (euclideanHeatKernel 3 (T-s)) z
          (EuclideanSpace.single i 1)) y (EuclideanSpace.single j 1) * F (s,x-y) =
          H (T-s) y * F (s,x-y)
        rw [hderiv y])
    have hkernel_meas : Measurable (fun p : ℝ × E3 =>
        H (T-p.1) p.2 * F (p.1,x-p.2)) := by
      ...
      have mf : Measurable (fun p : ℝ × E3 => F (p.1,x-p.2)) :=
        hF.measurable.comp (measurable_fst.prodMk (measurable_const.sub measurable_snd))
      exact (ma.mul mp).mul mf
    ...
    have hwint : IntervalIntegrable w volume 0 T := by
      simpa [w] using hwt.const_mul (C₀ * L)
    have hgbound : ∀ᵐ s ∂volume, s ∈ Ioc (0 : ℝ) T →
      ...
      let fs : E3 →ᵇ ℝ :=
        ⟨⟨fun y : E3 => F (s,y), hFs⟩, ⟨2 * B, by
          intro u v
          rw [Real.dist_eq]
          calc
            |F (s,u) - F (s,v)| ≤ |F (s,u)| + |F (s,v)| :=
              abs_sub _ _
            _ ≤ B + B := add_le_add
              (hFbound s ⟨le_of_lt hs.1, hs.2⟩ u)
              (hFbound s ⟨le_of_lt hs.1, hs.2⟩ v)
            _ = 2 * B := by ring⟩⟩
      have hholder_s : ∀ u v : E3,
          |fs u - fs v| ≤ L * ‖u-v‖^alpha := by
        intro u v
        change |F (s,u) - F (s,v)| ≤ L * ‖u-v‖^alpha
        exact hHolder s ⟨le_of_lt hs.1, hs.2⟩ u v
      obtain ⟨_, hb⟩ := hspatial fs L hL hholder_s
        (T-s) (sub_pos.mpr hslt) i j x
      change ‖g₀ s‖ ≤ w s
      rw [Real.norm_eq_abs, ← hconvolution_eq s hs hst]
      change |∫ y : E3, fderiv ℝ (fun z : E3 => fderiv ℝ
        (euclideanHeatKernel 3 (T-s)) z (EuclideanSpace.single i 1)) y
          (EuclideanSpace.single j 1) * fs (x-y)| ≤
        C₀ * L * (T-s) ^ (alpha / 2 - 1)
      exact hb
