import MorganTianLib.Ch03.RicciFlow.PDE.MetricCoefficients.FullBackgroundNonlinearity
...
/-- **Math.** background holder control. -/
theorem background_holder_control (A : E3 →L[ℝ] E3) (P : Fin 3 → E3 →L[ℝ] E3) (T : T3) (R : Fin 3 → T3)
    (c : ℝ) (hc : 0 < c) (hA : ∀ w : E3, c*‖w‖^2 ≤ inner ℝ (A w) w) :
    ∃ r K : ℝ, 0 < r ∧ 0 < K ∧ ∀ (u v : E3 → JBg),
      (∀ x, u x ∈ Metric.ball ((A,P),(T,R)) r) → (∀ x, v x ∈ Metric.ball ((A,P),(T,R)) r) →
      ∀ (alpha H D delta : ℝ), 0 ≤ H → 0 ≤ D → 0 ≤ delta →
      (∀ x, ‖u x-v x‖ ≤ delta) →
      (∀ x y, ‖u x-u y‖ ≤ H*‖x-y‖^alpha) → (∀ x y, ‖v x-v y‖ ≤ H*‖x-y‖^alpha) →
      (∀ x y, ‖(u x-v x)-(u y-v y)‖ ≤ D*‖x-y‖^alpha) →
      ∀ x y, ‖(fullBackgroundNonlinearity (u x)-fullBackgroundNonlinearity (v x))-
        (fullBackgroundNonlinearity (u y)-fullBackgroundNonlinearity (v y))‖ ≤ K*(D+H*delta)*‖x-y‖^alpha :=
/- SWARM_PROOF_BEGIN -/
by
  classical
  letI : NormedAddCommGroup E9 := inferInstance
  letI : InnerProductSpace ℝ E9 := inferInstance
  have hsmooth := full_background_nonlinearity_smooth A P T R c hc hA
  let q₀ : JBg := ((A, P), (T, R))
  obtain ⟨r, M, L, hr, hM, hL, hderiv, hLip⟩ :=
    local_C2_derivative_control fullBackgroundNonlinearity q₀
      (hsmooth.of_le (by norm_cast))
  let s : Set JBg := Metric.ball q₀ r
  ...
  · intro u v hu hv alpha H D delta hH hD hdelta hclose huHolder hvHolder hdiffHolder
    have hmain := holder_nonlinearity_difference
      (E := JBg) (F := E9) fullBackgroundNonlinearity s hsconvex
    ...
    have hcoeff : M * D + 2 * L * H * delta ≤ (M + 2 * L) * (D + H * delta) := by
      have hmhδ : 0 ≤ M * (H * delta) :=
        mul_nonneg (le_of_lt hM) (mul_nonneg hH hdelta)
      have h2ld : 0 ≤ (2 * L) * D :=
        mul_nonneg (mul_nonneg (by norm_num : (0 : ℝ) ≤ 2) (le_of_lt hL)) hD
      nlinarith
    intro x y
    calc
      ‖(fullBackgroundNonlinearity (u x) - fullBackgroundNonlinearity (v x)) -
          (fullBackgroundNonlinearity (u y) - fullBackgroundNonlinearity (v y))‖
